...
const products = rawProducts
    ...
    const combinedPrompt = `El usuario pregunto: "${extrationlanguagenatural}". Aqui hay una lista de productos relacionados:\n${productListText}\nPor favor, genera una descripción o recomendación para estos productos, uno por uno, haz una division por producto usando estos simbolos juntos "#1#" al final de cada descripcion de producto (por que mas adalante voy a dividir el prompt), recuerda estas hablando con el cliente directamente no conmigo(comienza dando una presentación demasiado corta), anuncia el producto mencionando sus beneficios y si no lo conoces da detalles lo mas precisos como si lo conocieras, si no hay nada en la lista de productos, solo manda "No hay ninguna descripción", recuerda son 8 productos los que se mandan, que no se te pase ninguno de mencionarlo.`; // Combinar el prompt del usuaario con la lista de productos
